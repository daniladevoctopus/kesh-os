    ...
    IN LPARAM lParam)
{
    ...
    switch (uMsg)
    {
        ...
        case WM_CTLCOLORDLG:
        {
            ...
            if (!hDlgBrush)
                hDlgBrush = CreateSolidBrush(RGB(252, 248, 242));
        ...
        case WM_CTLCOLORSTATIC:
        {
            ...
            SetTextColor(hdcStatic, RGB(56, 34, 22));
            return (INT_PTR)hDlgBrush;
        }
        ...
        case WM_PAINT:
        {
            ...
            if (pDlgData && pDlgData->hLogoBitmap)
            {
                DrawStateW(hdc, NULL, NULL, (LPARAM)pDlgData->hLogoBitmap, (WPARAM)0, 0, 0, 0, 0, DST_BITMAP);
            }
            RECT rc;
            GetClientRect(hwndDlg, &rc);
            HPEN hPen = CreatePen(PS_SOLID, 2, RGB(180, 130, 75));
            HPEN hOld = (HPEN)SelectObject(hdc, hPen);
            MoveToEx(hdc, 0, 72, NULL);
            LineTo(hdc, rc.right, 72);
