...
SetAutoAdminLogon(VOID)
{
    ...
    /* Always ensure CAD is disabled so users aren't trapped on Ctrl-Alt-Delete screen */
    RegSetValueExW(hKey,
                   L"DisableCAD",
                   0,
                   REG_DWORD,
                   (const BYTE*)&dwDisableCAD,
                   sizeof(dwDisableCAD));

    LPCWSTR szDomain = (AdminInfo.Domain && AdminInfo.Domain[0]) ? AdminInfo.Domain : L"WORKGROUP";
    LPCWSTR szUser   = (AdminInfo.Name && AdminInfo.Name[0]) ? AdminInfo.Name : L"Administrator";
    LPCWSTR szPass   = (AdminInfo.Password) ? AdminInfo.Password : L"";
    ...
    RegSetValueExW(hKey,
                   ...
                   REG_SZ,
                   (const BYTE*)szDomain,
                   (wcslen(szDomain) + 1) * sizeof(WCHAR));
