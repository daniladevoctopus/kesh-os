    ...
    _In_ LPCWSTR szSection)
{
    ...
    do
    {
        ...
        if (hmod != NULL)
        {
            ...
            FreeLibrary(hmod);
            DPRINT1("RegisterTypeLibraries: Registered resources for '%S'\n", szName);
        }
        else
        {
            DPRINT1("RegisterTypeLibraries: Skipping non-PE/unsupported '%S'\n", szName);
        }
...
CommonInstall(VOID)
{
    ...
    /* Wait up to 10 seconds for PnP device events so setup never hangs indefinitely */
    if (CMP_WaitNoPendingInstallEvents(10000) != WAIT_OBJECT_0)
    {
        DPRINT1("CMP_WaitNoPendingInstallEvents() timed out or failed, proceeding with setup...\n");
    }
...
InstallLiveCD(VOID)
{
    ...
    DPRINT1("InstallLiveCD: Registration finished, closing inf...\n");
    SetupCloseInfFile(hSysSetupInf);
    ...
    DPRINT1("InstallLiveCD: Launching userinit.exe...\n");
    ...
    if (!bRes)
    {
        DPRINT1("InstallLiveCD: CreateProcess(userinit.exe) failed: %lu\n", GetLastError());
        goto error;
    }
    DPRINT1("InstallLiveCD: userinit.exe started successfully! PID=%lu\n", ProcessInformation.dwProcessId);
...
SaveDefaultUserHive(VOID)
{
    ...
    RtlAdjustPrivilege(SE_BACKUP_PRIVILEGE, TRUE, FALSE, &OldBackup);
    RtlAdjustPrivilege(SE_RESTORE_PRIVILEGE, TRUE, FALSE, &OldRestore);
    ...
    /* Fallback: Guarantee that default user hive file exists */
    if (GetFileAttributesW(szDefaultUserHive) == INVALID_FILE_ATTRIBUTES)
    {
        WCHAR szSysDefault[MAX_PATH];
        GetSystemDirectoryW(szSysDefault, ARRAYSIZE(szSysDefault));
        wcscat(szSysDefault, L"\\config\\default");
        if (CopyFileW(szSysDefault, szDefaultUserHive, FALSE))
        {
...
InstallReactOS(VOID)
{
    ...
    /* Flush and commit all configuration to Default User hive */
    SaveDefaultUserHive();
    ...
    _SEH2_EXCEPT(EXCEPTION_EXECUTE_HANDLER)
    {
        DPRINT1("Exception freeing AdminInfo\n");
    }
    _SEH2_END;

Quit:
    /* Suppress GPF error dialogs during reboot/shutdown */
    ...
    /* Fallback in case direct syscall is blocked */
