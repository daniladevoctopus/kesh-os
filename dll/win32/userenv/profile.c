    ...
    _In_ BOOL bWin9xUpg)
{
    ...
    if (!CreateDirectoryW(szUserProfilePath, NULL))
    {
        ...
        WCHAR szCheckDat[MAX_PATH];
        StringCbCopyW(szCheckDat, sizeof(szCheckDat), szUserProfilePath);
        StringCbCatW(szCheckDat, sizeof(szCheckDat), L"\\ntuser.dat");
        if (GetFileAttributesW(szCheckDat) != INVALID_FILE_ATTRIBUTES)
        {
            for (i = 0; i < 1000; i++)
            {
    ...
    _Inout_ LPPROFILEINFOW lpProfileInfo)
{
    ...
    else
    {
         ...
         */
        WCHAR szProfileRegKey[MAX_PATH];
        HKEY hProfileEntryKey = NULL;
        BOOL bFoundInRegistry = FALSE;

        StringCbCopyW(szProfileRegKey, sizeof(szProfileRegKey),
                      L"SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\ProfileList\\");
        StringCbCatW(szProfileRegKey, sizeof(szProfileRegKey), SidString.Buffer);

        if (RegOpenKeyExW(HKEY_LOCAL_MACHINE, szProfileRegKey, 0, KEY_QUERY_VALUE, &hProfileEntryKey) == ERROR_SUCCESS)
        {
            WCHAR szRawImagePath[MAX_PATH];
            DWORD dwType = 0;
            DWORD cbData = sizeof(szRawImagePath);
            if (RegQueryValueExW(hProfileEntryKey, L"ProfileImagePath", NULL, &dwType, (LPBYTE)szRawImagePath, &cbData) == ERROR_SUCCESS)
            {
                ExpandEnvironmentStringsW(szRawImagePath, szUserHivePath, ARRAYSIZE(szUserHivePath));
                StringCbCatW(szUserHivePath, sizeof(szUserHivePath), L"\\ntuser.dat");
                bFoundInRegistry = TRUE;
            }
            RegCloseKey(hProfileEntryKey);
        }
        ...
        /* Ensure target hive file exists before loading */
        if (GetFileAttributesW(szUserHivePath) == INVALID_FILE_ATTRIBUTES)
        {
            WCHAR szDefHive[MAX_PATH];
            DWORD cchDef = ARRAYSIZE(szDefHive);
            if (GetDefaultUserProfileDirectoryW(szDefHive, &cchDef))
            {
                StringCbCatW(szDefHive, sizeof(szDefHive), L"\\ntuser.dat");
                CopyFileW(szDefHive, szUserHivePath, FALSE);
            }
            if (GetFileAttributesW(szUserHivePath) == INVALID_FILE_ATTRIBUTES)
            {
                WCHAR szSysDef[MAX_PATH];
                GetSystemDirectoryW(szSysDef, ARRAYSIZE(szSysDef));
                StringCbCatW(szSysDef, sizeof(szSysDef), L"\\config\\default");
                CopyFileW(szSysDef, szUserHivePath, FALSE);
            }
        }
