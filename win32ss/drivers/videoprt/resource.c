   ...
   OUT VP_STATUS *Status)
{
   ...
   {
      /* Fallback for standard x86 PC architecture where memory space is identity-mapped */
      if ((InIoSpace & VIDEO_MEMORY_SPACE_IO) == 0)
      {
         INFO_(VIDEOPRT, "HalTranslateBusAddress failed for %I64x, using 1:1 physical address\n", IoAddress.QuadPart);
