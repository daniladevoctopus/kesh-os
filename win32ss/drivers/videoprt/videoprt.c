    ...
    _In_ BOOLEAN LegacyDetection)
{
    ...
    if (LegacyDetection)
    {
        ...
        /* If bus enumeration failed to locate any bus of this type (such as under
         * UEFI where ISA/PCI buses are not populated in the ARC hardware tree),
         * fall back to probing bus 0 directly so legacy drivers can still initialize. */
        if (vpStatus == ERROR_DEV_NOT_EXIST)
        {
            DeviceExtension->SystemIoBusNumber =
