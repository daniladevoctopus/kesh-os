...
LaunchSecondStageLoader(VOID)
{
    ...
    if (!LoadRosload("rosload.exe", &ImageBase, &RosloadDTE) &&
        !LoadRosload("loader\\rosload.exe", &ImageBase, &RosloadDTE))
    {
        ULONG d, p, cd;
        BOOLEAN Found = FALSE;
        CHAR SavedPath[MAX_PATH];
        RtlStringCbCopyA(SavedPath, sizeof(SavedPath), FrLdrBootPath);

        /* 1. Try CD-ROM drives first (instant for ISO, CD, and Ventoy emulated optical drives) */
        for (cd = 0; cd < 4; cd++)
        {
            ...
            if (LoadRosload("loader\\rosload.exe", &ImageBase, &RosloadDTE) ||
                LoadRosload("rosload.exe", &ImageBase, &RosloadDTE))
            {
                Found = TRUE;
                break;
            }
        ...
        /* 2. Try hard disk and USB partitions (multi(0)disk(0)rdisk(d)partition(p)) */
        if (!Found)
        {
            for (d = 0; d < 4 && !Found; d++)
            {
                ...
                if (!Found)
                {
                    /* Try unpartitioned superfloppy (partition 0) */
                    RtlStringCbPrintfA(FrLdrBootPath, sizeof(FrLdrBootPath),
        ...
        if (!Found)
        {
            RtlStringCbCopyA(FrLdrBootPath, sizeof(FrLdrBootPath), SavedPath);
            return ENOENT;
        }
    }
