...
BOOLEAN IniFileInitialize(VOID)
{
    ...
    if (Status != ESUCCESS)
        Status = FsOpenFile("loader\\freeldr.ini", FrLdrBootPath, OpenReadOnly, &FileId);
    if (Status != ESUCCESS)
    ...
    if (Status != ESUCCESS)
    {
        ...
        /* Search hard disks and USB partitions */
        for (d = 0; d < 4 && Status != ESUCCESS; d++)
        {
            for (p = 1; p <= 4; p++)
            {
                RtlStringCbPrintfA(CandidatePath, sizeof(CandidatePath),
                                   "multi(0)disk(0)rdisk(%lu)partition(%lu)", d, p);
                Status = FsOpenFile("freeldr.ini", CandidatePath, OpenReadOnly, &FileId);
                if (Status != ESUCCESS)
                    Status = FsOpenFile("loader\\freeldr.ini", CandidatePath, OpenReadOnly, &FileId);
                if (Status == ESUCCESS)
                {
                    RtlStringCbCopyA(FrLdrBootPath, sizeof(FrLdrBootPath), CandidatePath);
                    break;
                }
            }
            if (Status != ESUCCESS)
            {
                RtlStringCbPrintfA(CandidatePath, sizeof(CandidatePath),
                                   "multi(0)disk(0)rdisk(%lu)partition(0)", d);
                Status = FsOpenFile("freeldr.ini", CandidatePath, OpenReadOnly, &FileId);
                if (Status != ESUCCESS)
                    Status = FsOpenFile("loader\\freeldr.ini", CandidatePath, OpenReadOnly, &FileId);
                if (Status == ESUCCESS)
        ...
        /* Search CD-ROM drives */
        for (cd = 0; cd < 4 && Status != ESUCCESS; cd++)
        {
            RtlStringCbPrintfA(CandidatePath, sizeof(CandidatePath),
                               "multi(0)disk(0)cdrom(%lu)", cd);
            Status = FsOpenFile("freeldr.ini", CandidatePath, OpenReadOnly, &FileId);
            if (Status != ESUCCESS)
                Status = FsOpenFile("loader\\freeldr.ini", CandidatePath, OpenReadOnly, &FileId);
            if (Status == ESUCCESS)
    ...
    if (Status != ESUCCESS)
    {
        ERR("Error while opening freeldr.ini/boot.ini, Status: %d\n", Status);
