    ...
    _Out_ PCONFIGURATION_TYPE DeviceType)
{
    ...
    *DeviceType = 0;
    ...
    else if (FrldrBootPartition == 0xFF)
    {
        ...
        UCHAR CdDrive = DiskFindElToritoDrive();
        if (CdDrive != 0)
            FrldrBootDrive = CdDrive;
#endif
    ...
    else if (FrldrBootDrive >= FIRST_BIOS_DISK)
    {
        ...
        {
            /* If no partition is marked active (common on USB flash drives), check if partition 1 exists */
            if (DiskGetPartitionEntry(FrldrBootDrive, 512, 1, &PartInfo))
            {
                BootPartition = 1;
            }
            else
            {
#if !defined(SARCH_XBOX) && !defined(SARCH_PC98)
                /* Check if we were booted from an El Torito CD-ROM */
                UCHAR CdDrive = DiskFindElToritoDrive();
                if (CdDrive != 0)
                {
                    FrldrBootDrive = CdDrive;
...
#endif
                /* Fallback to raw unpartitioned disk / superfloppy (partition 0) */
                BootPartition = 0;
            }
        }
    ...
    else if (FrldrBootDrive > 0 && FrldrBootDrive < FIRST_BIOS_DISK)
    {
        /* This is a floppy */
        RtlStringCbPrintfA(FrLdrBootPath, sizeof(FrLdrBootPath),
