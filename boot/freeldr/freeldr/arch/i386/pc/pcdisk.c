...
UCHAR
DiskFindElToritoDrive(VOID)
{
    REGS RegsIn, RegsOut;
    UCHAR Drive;
    PUCHAR Buf = (PUCHAR)BIOSCALLBUFFER;
    static const UCHAR CandidateDrives[] = { 0x81, 0x80, 0x82, 0x83, 0x84, 0x9F, 0x90, 0x8A, 0xE0 };

    DiskReportError(FALSE);

    /* First, check BIOS El Torito emulation status (INT 13h AX=4B01h) */
    for (ULONG i = 0; i < sizeof(CandidateDrives); i++)
    {
        Drive = CandidateDrives[i];
        memset(Buf, 0, 0x18);
        Buf[0] = 0x13;
        ...
        Int386(0x13, &RegsIn, &RegsOut);
        if (INT386_SUCCESS(RegsOut) && (RegsOut.b.ah == 0) && (Buf[0] >= 0x13))
        {
            if (Buf[2] == Drive || Buf[3] == Drive)
            {
                DiskReportError(TRUE);
                TRACE("Found El Torito boot CD-ROM drive: 0x%02X\n", Drive);
                return Drive;
            }
        }
    }

    /* Second: scan candidate drives for ISO-9660 Primary Volume Descriptor ("CD001" at LBA 16) */
    for (ULONG i = 0; i < sizeof(CandidateDrives); i++)
    {
        Drive = CandidateDrives[i];
        if (DiskInt13ExtensionsSupported(Drive))
        {
            if (PcDiskReadLogicalSectorsLBA(Drive, 16ULL, 1, DiskReadBuffer))
            {
                PUCHAR Pvd = (PUCHAR)DiskReadBuffer;
                if (Pvd[0] == 1 && Pvd[1] == 'C' && Pvd[2] == 'D' && Pvd[3] == '0' && Pvd[4] == '0' && Pvd[5] == '1')
                {
                    DiskReportError(TRUE);
                    TRACE("Found boot CD-ROM drive via PVD: 0x%02X\n", Drive);
                    return Drive;
                }
            }
        }
    ...
    OUT PVOID Buffer)
{
    ...
    for (RetryCount = 0; RetryCount < 5; ++RetryCount)
    {
        ...
        /* It failed, reset the controller and wait briefly */
        DiskResetController(DriveNumber);
        StallExecutionProcessor(20000);
    }

    /* If multi-sector read failed, fallback to reading sector-by-sector */
    if (SectorCount > 1)
    {
        PPC_DISK_DRIVE DiskDrive = PcDiskDriveNumberToDrive(DriveNumber);
        ULONG BytesPerSector = (DiskDrive && DiskDrive->ExtGeometry.BytesPerSector) ?
                               DiskDrive->ExtGeometry.BytesPerSector : 2048;
        PUCHAR CurrentBuffer = (PUCHAR)Buffer;

        for (ULONG i = 0; i < SectorCount; i++)
        {
            if (!PcDiskReadLogicalSectorsLBA(DriveNumber, SectorNumber + i, 1, CurrentBuffer))
            {
                return FALSE;
            }
            CurrentBuffer += BytesPerSector;
        }
        return TRUE;
    }

    /* If BIOS returned 0x0C (unsupported track / end of media on CD-ROM),
     * treat it gracefully as end of media: zero the buffer and succeed! */
    if (RegsOut.b.ah == 0x0C)
    {
        PPC_DISK_DRIVE DiskDrive = PcDiskDriveNumberToDrive(DriveNumber);
        ULONG BytesPerSector = (DiskDrive && DiskDrive->ExtGeometry.BytesPerSector) ?
                               DiskDrive->ExtGeometry.BytesPerSector : 2048;
        RtlZeroMemory(Buffer, SectorCount * BytesPerSector);
        return TRUE;
    }

    /* If we get here then the read really failed */
    {
        CHAR Msg[128];
        RtlStringCbPrintfA(Msg, sizeof(Msg),
                           "Disk Read Failed in LBA mode\nDrive: 0x%02X LBA: %I64u Count: %u",
                           DriveNumber, SectorNumber, SectorCount);
        DiskError(Msg, RegsOut.b.ah);
    }
