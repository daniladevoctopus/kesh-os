                    ...
                    OUT PANSI_STRING OutputString OPTIONAL)
{
    ...
    for (i = 0; i < KiBugCodeMessages->NumberOfBlocks; i++)
    {
            ...
            (BugCheckCode <= KiBugCodeMessages->Blocks[i].HighId))
        {
            ...
            /* Support both ANSI and Unicode message entries without asserting */
            static CHAR KiBugCodeConversionBuffer[1024];
            if (MessageEntry->Flags & MESSAGE_RESOURCE_UNICODE)
            {
                PWCHAR WideText = (PWCHAR)MessageEntry->Text;
                ULONG j = 0;
                while ((WideText[j] != UNICODE_NULL) && (j < sizeof(KiBugCodeConversionBuffer) - 1))
                {
                    KiBugCodeConversionBuffer[j] = (WideText[j] < 0x80) ? (CHAR)WideText[j] : '?';
                    j++;
                }
                KiBugCodeConversionBuffer[j] = ANSI_NULL;
                BugCode = KiBugCodeConversionBuffer;
                Length = (USHORT)j;
            }
            else
            {
                    ...
                    IN PCHAR Message)
{
    ...
    /* Print immediate STOP code so physical monitors always show the error code first */
