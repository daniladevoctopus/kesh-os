...
static PKTHREAD InbvLockOwner = NULL;
static ULONG InbvLockRecursion = 0;
...
InbvAcquireLock(VOID)
{
    ...
    /* Protect against recursive acquisition during screen debugging or interrupts */
    if (CurrentThread && InbvLockOwner == CurrentThread)
    {
        InbvLockRecursion++;
        return;
    }
...
InbvReleaseLock(VOID)
{
    ...
    /* Handle nested acquisition */
    if (InbvLockRecursion > 1)
    {
        InbvLockRecursion--;
        return;
    }
