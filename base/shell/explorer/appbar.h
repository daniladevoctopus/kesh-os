...
typedef PVOID (WINAPI *PFN_SHLockShared)(HANDLE hData, DWORD dwProcessId);
typedef BOOL (WINAPI *PFN_SHUnlockShared)(PVOID pvData);

static inline PAPPBARDATAINTEROP
AppBar_LockOutput(_In_ PAPPBAR_COMMAND pData)
{
    static PFN_SHLockShared s_pfnLock = NULL;
    if (!s_pfnLock)
    {
        HMODULE hMod = GetModuleHandleW(L"shlwapi.dll");
        if (hMod)
            s_pfnLock = (PFN_SHLockShared)GetProcAddress(hMod, "SHLockShared");
        if (!s_pfnLock)
        {
    ...
    if (s_pfnLock)
        return (PAPPBARDATAINTEROP)s_pfnLock((HANDLE)pData->hOutput, pData->dwProcessId);
    return NULL;
}
...
{
    static PFN_SHUnlockShared s_pfnUnlock = NULL;
    if (!s_pfnUnlock)
    {
        HMODULE hMod = GetModuleHandleW(L"shlwapi.dll");
        if (hMod)
            s_pfnUnlock = (PFN_SHUnlockShared)GetProcAddress(hMod, "SHUnlockShared");
        if (!s_pfnUnlock)
        {
            hMod = GetModuleHandleW(L"shell32.dll");
            if (hMod)
