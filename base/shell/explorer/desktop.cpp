...
DWORD CDesktopThread::DesktopThreadProc()
{
    ...
    /* Ensure KeshOS wallpaper is active */
    {
        WCHAR szCurWp[MAX_PATH];
        szCurWp[0] = L'\0';
        if (!SystemParametersInfoW(SPI_GETDESKWALLPAPER, MAX_PATH, szCurWp, 0) || szCurWp[0] == L'\0')
        {
            WCHAR szKeshWp[MAX_PATH];
            if (GetWindowsDirectoryW(szKeshWp, MAX_PATH))
            {
                PathAppendW(szKeshWp, L"keshos.bmp");
                if (GetFileAttributesW(szKeshWp) != INVALID_FILE_ATTRIBUTES)
                {
                    SystemParametersInfoW(SPI_SETDESKWALLPAPER, 0, szKeshWp, SPIF_UPDATEINIFILE | SPIF_SENDCHANGE);
                }
            }
        }
    }
