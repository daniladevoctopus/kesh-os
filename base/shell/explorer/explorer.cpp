...
StartWithCommandLine(IN HINSTANCE hInstance)
{
    ...
    if (hBrowseUI)
    {
        typedef BOOL (WINAPI *PFN_SHExplorerParseCmdLine)(EXPLORER_CMDLINE_PARSE_RESULTS *);
        typedef BOOL (WINAPI *PFN_SHCreateFromDesktop)(EXPLORER_CMDLINE_PARSE_RESULTS *);
        PFN_SHExplorerParseCmdLine pfnParse = (PFN_SHExplorerParseCmdLine)GetProcAddress(hBrowseUI, (LPCSTR)107);
        PFN_SHCreateFromDesktop pfnCreate = (PFN_SHCreateFromDesktop)GetProcAddress(hBrowseUI, (LPCSTR)106);

        if (pfnParse && pfnParse(&parseResults))
        {
            if (pfnCreate)
                b = pfnCreate(&parseResults);
        }
...
StartWithDesktop(IN HINSTANCE hInstance)
{
...
#if !WIN7_COMPAT_MODE
    /* Initialize shell dde support only if no host shell is already running */
    ...
    BOOL bIsHostShellRunning = (GetShellWindow() != NULL);

#if !WIN7_DEBUG_MODE
    HANDLE hShellDesktop = NULL;
    if (!bIsHostShellRunning)
    {
          ...
          IN INT nCmdShow)
{
    ...
    */
    ...
    BOOL bForceDesktop = (lpCmdLine != NULL) &&
                         (StrStrIW(lpCmdLine, L"/desktop") != NULL ||
                          StrStrIW(lpCmdLine, L"-desktop") != NULL ||
                          StrStrIW(lpCmdLine, L"/test") != NULL ||
                          StrStrIW(lpCmdLine, L"-test") != NULL);

#if !WIN7_COMPAT_MODE
    bExplorerIsShell = (GetShellWindow() == NULL) && IsExplorerSystemShell();

    if (!bExplorerIsShell && !bForceDesktop)
    {
